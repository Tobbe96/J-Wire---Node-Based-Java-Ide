...
const DetailsPanel = ({ selectedNode, updateNodeModifier, updateNodeData }: DetailsPanelProps) => {
  ...
  const removeParam = (index: number) => {
    const params = data.parameters.filter((_: Parameter, i: number) => i !== index);
    updateNodeData(id, { parameters: params });
  };

  const updateLocal = (index: number, field: string, value: string) => {
    const locals = [...(data.localVariables || [])];
    locals[index] = { ...locals[index], [field]: value };
    updateNodeData(id, { localVariables: locals });
  };

  const removeLocal = (index: number) => {
    const locals = data.localVariables.filter((_: LocalVariable, i: number) => i !== index);
    updateNodeData(id, { localVariables: locals });
  };
    ...
    <div style={panelStyle}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', overflowY: 'auto', flex: 1 }}>
        <div style={inputGroupStyle}>
          ...
          <input style={inputStyle} value={data.label || ''} onChange={(e) => updateNodeData(id, { label: e.target.value })} />
        </div>
        ...
        {nodeType === 'java' && (
          <>
            <div style={inputGroupStyle}>
              ...
              <input style={inputStyle} value={data.value || ''} onChange={(e) => updateNodeData(id, { value: e.target.value })} />
            </div>
            <div style={inputGroupStyle}>
              <label style={labelStyle}>ACCESS MODIFIER</label>
              <select style={selectStyle} value={data.modifier || 'public'} onChange={(e) => updateNodeModifier(id, e.target.value)}>
          ...
          <>
            <div style={{ marginTop: '5px' }}>
              ...
              <div style={scrollAreaStyle}>
                {(data.parameters || []).map((p: Parameter, index: number) => (
            ...
            <div style={{ marginTop: '5px' }}>
              ...
              <div style={scrollAreaStyle}>
                {(data.localVariables || []).map((l: LocalVariable, index: number) => (
