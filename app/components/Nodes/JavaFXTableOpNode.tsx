...
const OP_LABELS: Record<string, string> = {
  create: 'Create', addColumn: 'Add Column', addRow: 'Add Row',
  setItems: 'Set Items', getSelectedItem: 'Get Selected', setEditable: 'Set Editable',
  setCellValueFactory: 'Cell Value Factory',
};
...
const JavaFXTableOpNode = ({ data, selected, id }: Props) => {
  const op = (data.operation as string) || 'create';
  const varName = (data.variableName as string) || 'table';
  const update = data.updateNodeData as ((i: string, d: Record<string, unknown>) => void) | undefined;

  const sub = (
    <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
      <input className="nodrag" value={varName} onChange={e => update?.(id, { variableName: e.target.value })} style={varInp} placeholder="tableName" />
  ...
  const wrap = (children: React.ReactNode) => (
    <div style={{ ...nodeContainer(FX_COLOR, !!selected), minWidth: '240px' }}>
      <div className="devflow-header-shimmer" style={nodeHeaderSolid(FX_COLOR)}>📋 TableView: {OP_LABELS[op] ?? op}</div>
  ...
  if (op === 'addColumn') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{EXEC_IN}</div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-name" style={{ ...dataHandleStyle(STR_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>colName: String</span></div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-prop" style={{ ...dataHandleStyle(STR_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>property: String</span></div>
      </div>
      <div style={row}>{EXEC_OUT}</div>
    </div>
  );
  ...
  if (op === 'getSelectedItem') return wrap(
    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
      <div style={row}><span style={{ ...lbl, marginRight: '5px' }}>item</span><Handle type="source" position={Position.Right} id="data-out" style={{ ...dataHandleStyle(FX_COLOR, 'right'), right: '-6px' }} /></div>
    </div>
  );

  if (op === 'setEditable') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{EXEC_IN}</div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-value" style={{ ...dataHandleStyle(BOOL_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>editable: boolean</span></div>
      </div>
      <div style={row}>{EXEC_OUT}</div>
    </div>
  );

  if (op === 'setCellValueFactory') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{EXEC_IN}</div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-name" style={{ ...dataHandleStyle(STR_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>property: String</span></div>
      </div>
      <div style={row}>{EXEC_OUT}</div>
    </div>
  );

  if (op === 'addRow') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{EXEC_IN}</div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-item" style={{ ...dataHandleStyle(FX_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>item: Object</span></div>
      </div>
      <div style={row}>{EXEC_OUT}</div>
    </div>
  );
...
export default memo(JavaFXTableOpNode);
