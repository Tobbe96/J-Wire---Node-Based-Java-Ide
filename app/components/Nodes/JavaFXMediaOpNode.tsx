...
const OP_LABELS: Record<string, string> = {
  createImageView: 'Create ImageView', setImage: 'Set Image', setFitWidth: 'Fit Width', setFitHeight: 'Fit Height',
  createMediaPlayer: 'Create MediaPlayer', createMediaView: 'Create MediaView',
...
const JavaFXMediaOpNode = ({ data, selected, id }: Props) => {
  const op = (data.operation as string) || 'createImageView';
  const varName = (data.variableName as string) || 'media';
  const update = data.updateNodeData as ((i: string, d: Record<string, unknown>) => void) | undefined;

  const sub = (
    <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
      <input className="nodrag" value={varName} onChange={e => update?.(id, { variableName: e.target.value })} style={varInp} placeholder="mediaName" />
  ...
  const wrap = (children: React.ReactNode) => (
    <div style={{ ...nodeContainer(FX_COLOR, !!selected), minWidth: '240px' }}>
      <div className="jwire-header-shimmer" style={nodeHeaderSolid(FX_COLOR)}>🎬 Media: {OP_LABELS[op] ?? op}</div>
      <div style={{ padding: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>{sub}{children}</div>
    </div>
  );

  if (REF_OPS.has(op)) return wrap(
    op === 'createMediaPlayer' ? (
      <div style={between}>
        <div style={col}>
          <div style={row}>{EXEC_IN}</div>
          <div style={row}><Handle type="target" position={Position.Left} id="data-in-path" style={{ ...dataHandleStyle(STR_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>path: String</span></div>
        </div>
        <div style={col}>
...
export default memo(JavaFXMediaOpNode);
