...
const OP_LABELS: Record<string, string> = {
  create: 'Create BST', insert: 'Insert', delete: 'Delete', search: 'Search',
  min: 'Min', max: 'Max', height: 'Height', size: 'Size', contains: 'Contains',
...
const BSTOpNode = ({ data, selected, id }: Props) => {
  const op = (data.operation as string) || 'create';
  const valueType = (data.valueType as string) || 'int';
  const varName = (data.variableName as string) || 'bstRoot';
  ...
  // search / contains → boolean
  if (BOOL_OPS.has(op)) return wrap(
    <div style={between}>
      <div style={row}><Handle type="target" position={Position.Left} id="data-in-value" style={{ ...dataHandleStyle(valColor, 'left'), left: '-6px' }} /><span style={lbl}>value: {valueType}</span></div>
  ...
  // inorder / preorder / postorder → ArrayList<T>
  return wrap(
    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
