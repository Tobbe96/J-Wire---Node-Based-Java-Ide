...
const OP_LABELS: Record<string, string> = {
  create: 'Create Node', getValue: 'Get Value', setValue: 'Set Value',
  getLeft: 'Get Left', setLeft: 'Set Left', getRight: 'Get Right', setRight: 'Set Right',
  isNull: 'Is Null?', hasLeft: 'Has Left?', hasRight: 'Has Right?',
};

type Props = { id: string; data: Record<string, unknown>; selected?: boolean };

const TreeNodeOpNode = ({ data, selected, id }: Props) => {
  ...
  const sub = (
    <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
      ...
      {(op === 'create' || op === 'getValue' || op === 'setValue') && (
  ...
  const wrap = (children: React.ReactNode) => (
    <div style={{ ...nodeContainer(ACCENT, !!selected), minWidth: '200px' }}>
      <div className="jflow-header-shimmer" style={nodeHeaderSolid(ACCENT)}>🌿 TreeNode: {OP_LABELS[op] ?? op}</div>
      <div style={{ padding: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>{sub}{children}</div>
    </div>
  );

  // create: exec-in, data-in-value → exec-out
  ...
  // setValue / setLeft / setRight: exec-chain ops
  ...
  if (op === 'setLeft' || op === 'setRight') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{execL}</div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-node" style={{ ...dataHandleStyle(TREE_NODE_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>node: TreeNode</span></div>
      </div>
      <div style={row}>{execR}</div>
    </div>
  );

  // getValue → data-out (valueType)
  if (op === 'getValue') return wrap(
    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
      <div style={row}><span style={{ ...lbl, marginRight: '5px' }}>{valueType}</span><Handle type="source" position={Position.Right} id="data-out" style={{ ...dataHandleStyle(valColor, 'right'), right: '-6px' }} /></div>
    </div>
  );

  // getLeft / getRight → data-out (TreeNode)
  if (op === 'getLeft' || op === 'getRight') return wrap(
    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
      <div style={row}><span style={{ ...lbl, marginRight: '5px' }}>TreeNode</span><Handle type="source" position={Position.Right} id="data-out" style={{ ...dataHandleStyle(TREE_NODE_COLOR, 'right'), right: '-6px' }} /></div>
    </div>
  );

  // isNull / hasLeft / hasRight → data-out (boolean)
  if (op === 'isNull' || op === 'hasLeft' || op === 'hasRight') return wrap(
    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
...
export default memo(TreeNodeOpNode);
