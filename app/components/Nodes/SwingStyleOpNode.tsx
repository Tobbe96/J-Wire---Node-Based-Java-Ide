...
const OP_LABELS: Record<string, string> = {
  setFont: 'Set Font', setForeground: 'Set Foreground', setBackground: 'Set Background',
  setPreferredSize: 'Set Preferred Size', setBorder: 'Set Border',
  setToolTipText: 'Set Tooltip', setOpaque: 'Set Opaque',
};
...
const SwingStyleOpNode = ({ data, selected, id }: Props) => {
  const op = (data.operation as string) || 'setFont';
  const varName = (data.variableName as string) || 'comp';
  ...
  const wrap = (children: React.ReactNode) => (
    <div style={{ ...nodeContainer(SWING_COLOR, !!selected), minWidth: '230px' }}>
      <div className="devflow-header-shimmer" style={nodeHeaderSolid(SWING_COLOR)}>🎨 Style: {OP_LABELS[op] ?? op}</div>
      <div style={{ padding: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>{sub}{children}</div>
    </div>
  );

  if (op === 'setFont') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{EXEC_IN}</div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-value" style={{ ...dataHandleStyle(STR_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>font: String</span></div>
      </div>
      <div style={row}>{EXEC_OUT}</div>
    </div>
  );

  if (op === 'setForeground' || op === 'setBackground') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{EXEC_IN}</div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-value" style={{ ...dataHandleStyle(STR_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>color: String</span></div>
      </div>
      <div style={row}>{EXEC_OUT}</div>
    </div>
  );

  if (op === 'setPreferredSize') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{EXEC_IN}</div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-w" style={{ ...dataHandleStyle(NUM_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>w: int</span></div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-h" style={{ ...dataHandleStyle(NUM_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>h: int</span></div>
      </div>
      <div style={row}>{EXEC_OUT}</div>
    </div>
  );

  if (op === 'setBorder' || op === 'setToolTipText') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{EXEC_IN}</div>
...
export default memo(SwingStyleOpNode);
