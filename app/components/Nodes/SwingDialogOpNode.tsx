...
const OP_LABELS: Record<string, string> = {
  ...
  showInputDialog: 'Input Dialog', showOptionDialog: 'Option Dialog',
};
...
const SwingDialogOpNode = ({ data, selected, id }: Props) => {
  const op = (data.operation as string) || 'showMessageDialog';
  ...
  const wrap = (children: React.ReactNode) => (
    <div style={{ ...nodeContainer(SWING_COLOR, !!selected), minWidth: '230px' }}>
      <div className="devflow-header-shimmer" style={nodeHeaderSolid(SWING_COLOR)}>💬 Dialog: {OP_LABELS[op] ?? op}</div>
      <div style={{ padding: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>{sub}{children}</div>
    </div>
  );

  if (op === 'showMessageDialog') return wrap(
    <div style={between}>
      <div style={col}>
        ...
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-msg" style={{ ...dataHandleStyle(STR_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>msg: String</span></div>
      </div>
      <div style={row}>{EXEC_OUT}</div>
    </div>
  );

  if (op === 'showConfirmDialog') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{EXEC_IN}</div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-title" style={{ ...dataHandleStyle(STR_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>title: String</span></div>
      ...
      <div style={col}>
        <div style={row}>{EXEC_OUT}</div>
        <div style={row}><span style={{ ...lbl, marginRight: '5px' }}>result: int</span><Handle type="source" position={Position.Right} id="data-out" style={{ ...dataHandleStyle(NUM_COLOR, 'right'), right: '-6px' }} /></div>
      </div>
    </div>
  );

  if (op === 'showInputDialog') return wrap(
    <div style={between}>
      <div style={col}>
        <div style={row}>{EXEC_IN}</div>
        <div style={row}><Handle type="target" position={Position.Left} id="data-in-msg" style={{ ...dataHandleStyle(STR_COLOR, 'left'), left: '-6px' }} /><span style={lbl}>msg: String</span></div>
      ...
      <div style={col}>
        <div style={row}>{EXEC_OUT}</div>
        <div style={row}><span style={{ ...lbl, marginRight: '5px' }}>result: String</span><Handle type="source" position={Position.Right} id="data-out" style={{ ...dataHandleStyle(STR_COLOR, 'right'), right: '-6px' }} /></div>
      </div>
    </div>
  );
...
export default memo(SwingDialogOpNode);
