...
Build output is placed in the `installation/` directory.
...
app/
├── components/
│   ├── canvas/        # React Flow canvas components (nodes, edges, context menu)
│   ├── Nodes/         # 60+ individual node type components
│   ├── Panels/        # Sidebar panels (details, file tree, debug, terminal, etc.)
│   ├── ui/            # Reusable UI components (modals, toggles, toasts, etc.)
│   └── vfx/           # Visual effects (particles, sparks, ripples)
├── hooks/             # Custom React hooks
├── store/             # Zustand state stores
├── utils/             # Utilities, compiler, executor, node registry
├── api/               # Next.js API routes (compile endpoint)
├── page.tsx           # Main app entry point
└── layout.tsx         # Root layout
electron/              # Electron main process
installation/          # Build output (installers, generated by electron-builder)
docs/                  # Documentation
